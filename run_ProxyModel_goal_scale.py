...
def showModel(modeldata):
    ...
    practice_mean = np.zeros(len(parameters['goal_scale']))
    i = 0
    ...
    practice_std = np.zeros(len(parameters['goal_scale']))
    i = 0
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        runcolor = np.zeros([len(goal_scale_data.AgentID.unique()),
                             len(goal_scale_data.Run.unique())])
        for index, iteration in enumerate(goal_scale_data.Run.unique()):
            for Agent in goal_scale_data.AgentID.unique():
                proxy[Agent][index] = goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        goal_scale = modeldata[modeldata.Run == run].goal_scale.iloc[0]
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(goal_scale_data.Run.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == step)].Practice
...
#                for Agent in goal_scale_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = goal_scale_data[(goal_scale_data.Run == iteration) & (goal_scale_data.Step == step)].Talent.iloc[Agent]
