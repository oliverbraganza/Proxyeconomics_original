...
parameters = {"data_collect_interval": data_collect_interval,
              ...
              "practice_mutation_rate": np.linspace(0, np.pi/18, 9),
              "survival_uncertainty": 3}    # currently not in model (KT)

batch_run = BatchRunner(pm1.ProxyModel, parameters, iterations=10, max_steps=finalStep+1,
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        practice_mutation_rate = agentdata[agentdata.Run == run].practice_mutation_rate.iloc[0]
        ...
        proxy = np.zeros([len(practice_mutation_rate_data.AgentID.unique()),
                          len(practice_mutation_rate_data.Run.unique())])
        ...
        runcolor = np.zeros([len(practice_mutation_rate_data.AgentID.unique()),
                             len(practice_mutation_rate_data.Run.unique())])
        for index, iteration in enumerate(practice_mutation_rate_data.Run.unique()):
            for Agent in practice_mutation_rate_data.AgentID.unique():
                proxy[Agent][index] = practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        practice_mutation_rate = modeldata[modeldata.Run == run].practice_mutation_rate.iloc[0]
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(practice_mutation_rate_data.Run.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == step)].Practice
...
#                for Agent in practice_mutation_rate_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = practice_mutation_rate_data[(practice_mutation_rate_data.Run == iteration) & (practice_mutation_rate_data.Step == step)].Talent.iloc[Agent]
