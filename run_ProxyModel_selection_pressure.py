...
parameters = {"data_collect_interval": data_collect_interval,
              ...
              "goal_scale": 1,
              "goal_angle": np.pi/4,
              "selection_pressure": [0.01, 0.1, 0.2, 1],
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        selection_pressure = agentdata[agentdata.Run == run].selection_pressure.iloc[0]
        ...
        proxy = np.zeros([len(selection_pressure_data.AgentID.unique()),
                         len(selection_pressure_data.Run.unique())])
        ...
        runcolor = np.zeros([len(selection_pressure_data.AgentID.unique()),
                             len(selection_pressure_data.Run.unique())])
        for index, iteration in enumerate(selection_pressure_data.Run.unique()):
            for Agent in selection_pressure_data.AgentID.unique():
                proxy[Agent][index] = selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        selection_pressure = modeldata[modeldata.Run == run].selection_pressure.iloc[0]
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(selection_pressure_data.Run.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == step)].Practice
...
#                for Agent in selection_pressure_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = selection_pressure_data[(selection_pressure_data.Run == iteration) & (selection_pressure_data.Step == step)].Talent.iloc[Agent]
