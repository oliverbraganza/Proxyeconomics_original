...
variable_parameters = {"goal_angle": np.linspace(0, np.pi, 9)}
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        proxy = np.zeros([len(goal_angle_data.AgentID.unique()),
                         len(goal_angle_data.Run.unique())])
        ...
        runcolor = np.zeros([len(goal_angle_data.AgentID.unique()),
                             len(goal_angle_data.Run.unique())])
        for index, iteration in enumerate(goal_angle_data.Run.unique()):
            for Agent in goal_angle_data.AgentID.unique():
                proxy[Agent][index] = goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        goal_angle = modeldata[modeldata.Run == run].goal_angle.iloc[0]
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(goal_angle_data.Run.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == step)].Practice
...
#                for Agent in goal_angle_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == step)].Talent.iloc[Agent]
