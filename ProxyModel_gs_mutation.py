...
#            angle_list = neighbor_practices
        ...
        def get_utility(self, prospect):
            ''' utility function '''
            self.goal = np.cos(self.model.goal_angle - self.practice) * self.effort
            self.goal = np.cos(self.practice) * self.effort
            gsc = self.goal_scale
            e = self.effort
            t = self.talent

            utility = prospect + gsc*self.goal - (e**2)/t
            utility_gMR = prospect + gsc*self.proxy - (e**2)/t
...
class ProxyModel(Model):
    ...
    def kill_and_replace(self):
        ...
        potential_winners = list(winners for winners in agents if winners.proxy >= survival_threshold)
        for potential_loser in potential_losers:
            if np.random.rand() < self.selection_pressure:
                ...
                loser.goal_scale = np.random.normal(winner.goal_scale, 0.1)
