...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        runcolor = np.zeros([len(numAgents_data.AgentID.unique()),
                             len(numAgents_data.Run.unique())])
        for index, iteration in enumerate(numAgents_data.Run.unique()):
            for Agent in numAgents_data.AgentID.unique():
                proxy[Agent][index] = numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        numAgents = modeldata[modeldata.Run == run].numAgents.iloc[0]
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(numAgents_data.Run.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == step)].Practice
...
#                for Agent in numAgents_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = numAgents_data[(numAgents_data.Run == iteration) & (numAgents_data.Step == step)].Talent.iloc[Agent]
