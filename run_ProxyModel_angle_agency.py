...
finalStep = 100
variable_parameters = {"angle_agency": np.linspace(0.01, 0.09, 9)}
...
batch_run = BatchRunner(pm1.ProxyModel, variable_parameters, parameters, iterations=1, max_steps=finalStep+1,
...
def showModel(modeldata):
    ...
    ''' colormap '''
    cmap = plt.cm.jet
    cNorm = colors.Normalize(vmin=np.min(modeldata.angle_agency),
                             vmax=np.max(modeldata.angle_agency))
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cmap)
    low_col = scalarMap.to_rgba(min(modeldata.angle_agency))
    high_col = scalarMap.to_rgba(max(modeldata.angle_agency))

    ''' compute means & std '''
    xVals = modeldata.angle_agency.unique()
    Gm = modeldata.groupby(['angle_agency'])['mean_goal_value'].mean()
    Gstd = modeldata.groupby(['angle_agency'])['mean_goal_value'].std()
    Pm = modeldata.groupby(['angle_agency'])['mean_proxy_value'].mean()
    Pstd = modeldata.groupby(['angle_agency'])['mean_proxy_value'].std()

    ax1.scatter(modeldata.angle_agency, modeldata.mean_goal_value,
                alpha=0.5, c=low_col, marker=".", label='_nolegend_')
    ax1.scatter(modeldata.angle_agency, modeldata.mean_proxy_value,
    ...
    ax1.set_xlabel('angle_agency')

    utility_mean = modeldata.groupby(['angle_agency'])['mean_utility'].mean()
    utility_std = modeldata.groupby(['angle_agency'])['mean_utility'].std()
    ax2.scatter(modeldata.angle_agency, modeldata.mean_utility,
    ...
    ax2.set_xlabel('angle_agency')

    groups_m = modeldata.groupby(['angle_agency'])['mean_practice']
    ...
    for name, group in groups_m:
        ...
        i += 1
    groups_std = modeldata.groupby(['angle_agency'])['mean_practice']
    ...
    ax2a = ax2.twinx()
    ax2a.scatter(modeldata.angle_agency,
    ...
    ''' vector plot '''
    G_oc_m = modeldata.groupby(['angle_agency'])['mean_goal_oc'].mean()
    ...
    mpl.colorbar.ColorbarBase(cbar_ax, cmap=cmap, norm=cNorm,
                              orientation='vertical', label='angle_agency')
...
def showAgentDynamics(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        ax.set_xlabel('agent #')
        variable = run_data.angle_agency.iloc[0]
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        angle_agency = agentdata[agentdata.Run == run].angle_agency.iloc[0]
        angle_agency_data = agentdata[agentdata.angle_agency == angle_agency]
        proxy = np.zeros([len(angle_agency_data.AgentID.unique()),
                         len(angle_agency_data.Run.unique())])
        goal_oc = np.zeros([len(angle_agency_data.AgentID.unique()),
                           len(angle_agency_data.Run.unique())])
        talent = np.zeros([len(angle_agency_data.AgentID.unique()),
                          len(angle_agency_data.Run.unique())])
        runcolor = np.zeros([len(angle_agency_data.AgentID.unique()),
                             len(angle_agency_data.Run.unique())])
        for index, iteration in enumerate(angle_agency_data.Run.unique()):
            for Agent in angle_agency_data.AgentID.unique():
                proxy[Agent][index] = angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == visualizeStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == visualizeStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == visualizeStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    """ Model Dynamics for particular angle_agency """
    ...
    cNorm = colors.Normalize(vmin=np.min(modeldata.angle_agency),
                             vmax=np.max(modeldata.angle_agency))
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cmap)
    low_col = scalarMap.to_rgba(np.min(modeldata.angle_agency))
    high_col = scalarMap.to_rgba(np.max(modeldata.angle_agency))
    ...
    for i, ax in enumerate(ax_array):
        ...
        angle_agency = modeldata[modeldata.Run == run].angle_agency.iloc[0]
        angle_agency_data = modeldata[modeldata.angle_agency == angle_agency]

        xVals = angle_agency_data.Step.unique()
        proxy_mean = angle_agency_data.groupby(['Step'])['mean_proxy_value'].mean()
        proxy_std = angle_agency_data.groupby(['Step'])['mean_proxy_value'].std()
        # ax.scatter(angle_agency_data.Step,
        #           angle_agency_data.mean_proxy_value,
        ...
        goal_mean = angle_agency_data.groupby(['Step'])['mean_goal_value'].mean()
        goal_std = angle_agency_data.groupby(['Step'])['mean_goal_value'].std()
        # ax.scatter(angle_agency_data.Step,
        #           angle_agency_data.mean_goal_value,
        ...
        ax.set_xlabel('step')
        # ax.set_title('c{:.1f} r{:02d}'.format(angle_agency, run))
    ...
    for i, ax in enumerate(ax_array):
        ...
        run = int(i * (max(modeldata.Run)+1)/(columns-0.9))
        angle_agency = modeldata[modeldata.Run == run].angle_agency.iloc[0]
#        print(angle_agency/np.pi * 180)
        angle_agency_data = modeldata[modeldata.angle_agency == angle_agency]
        xVals = angle_agency_data.Step.unique()
        yVals_raw = angle_agency_data.groupby(['Step'])['mean_practice']
        ...
        practice_std = practice_std / np.pi * 180
#        ax.scatter(angle_agency_data.Step,
#                   angle_agency_data.mean_practice,
...
def showAgentProxyDynamics(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        run = int(i * (max(agentdata.Run)+1)/(columns-0.9))
        run_data = agentdata[agentdata.Run == run]
        # angle_agency = run_data.angle_agency.iloc[0]
        image = np.empty((len(run_data.Step.unique()),
                          len(run_data.AgentID.unique())))
...
def showSortedAgents(agentdata):
    ...
    # ga = agentdata.angle_agency.iloc[0]
    for i, ax in enumerate(ax_array):
        if i == 0:
            ax.set_ylabel('proxy')
        else:
        ...
        proxy = np.zeros([len(angle_agency_data.AgentID.unique()),
                          len(angle_agency_data.Run.unique()),
                          StepsToAverage])
        practice = np.zeros([len(angle_agency_data.AgentID.unique()),
                             len(angle_agency_data.Run.unique()),
                             StepsToAverage])
        talent = np.zeros([len(angle_agency_data.AgentID.unique()),
                           len(angle_agency_data.Run.unique()),
        ...
        for index, iteration in enumerate(angle_agency_data.Run.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == step)].Practice
...
#                for Agent in angle_agency_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = angle_agency_data[(angle_agency_data.Run == iteration) & (angle_agency_data.Step == step)].Talent.iloc[Agent]

        # plot goal angle
        # ax.plot([0, 45], [0, 0], c='grey', ls='--', lw=0.5)
        xVals = angle_agency_data.AgentID.unique()
